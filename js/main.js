...
var main = {
	...
	init : function() {
		...
		if ($("#scroll-box").length > 0
				&& Cookies.get('daScrollboxSubscribe3') === undefined) {
			if ($("article").length > 0) {
				main.scrollBoxCheck = $("article").offset().top
						+ $("article").height() * 0.4;
				$("#scroll-box-close").click(function() {
					$("#scroll-box").fadeOut(500);
					$("body").removeClass("scroll-box-on");
					Cookies.set('daScrollboxSubscribe3', '1', {
						expires : 1
					});
				});
			}
		...
		// On mobile, when clicking on a multi-level navbar menu, show the child
		// links
		...
		if (typeof ga === "function") {
			$("a[data-ga-event]").click(
					function() {
						ga('send', 'event', $(this).data("ga-category"),
								$(this).data("ga-action"), $(this).data(
										"ga-label"));
					});
		}
	},
	...
	initImgs : function() {
		...
		if ($("#header-big-imgs").length > 0) {
			...
			// For better UX, prefetch the next image so that it will already be
			// loaded when we want to show it
			var getNextImg = function() {
				...
				prefetchImg.src = src;
				// if I want to do something once the image is ready:
				...
				setTimeout(function() {
					...
					// after the animation of fading in the new image is done,
					// prefetch the next one
					// img.one("transitioned webkitTransitionEnd oTransitionEnd
				...
				if ($("meta[name='twitter:title'").length > 0) {
					...
					$.extend(addthis_share, {
						passthrough : {
							twitter : {
								via : "twittertwitter",
								text : trim($("meta[name='twitter:title'")
										.attr("content"), 93)
										+ " #rstats"
							}
						}
